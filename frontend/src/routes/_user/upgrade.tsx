...
function RouteComponent() {
  ...
  const _createPayment = useCreatePaymentLink()
  ...
  const totalPages = paymentHistory
    ? Math.ceil(paymentHistory.total / itemsPerPage)
    : 0
  ...
  useEffect(() => {
    ...
    if (paymentStatus) {
      ...
      if (paymentStatus === "success" && orderCode) {
        ...
        paymentsApi
          .syncPaymentStatusByCode(parseInt(orderCode, 10))
          .then((response) => {
            ...
            if (response.status === "PAID") {
              ...
              toast(
                "Payment is being processed. Please check again in a few minutes",
                { duration: 5000, icon: "⏳" },
              )
            }
          })
          .catch(() => {
            toast.dismiss(loadingToast)
            toast.error(
              "Unable to confirm payment. Please check transaction history",
              { duration: 5000 },
            )
          })
  ...
  const handlePaymentConfirm = async () => {
    ...
    try {
      ...
      toast.error(
        error?.body?.detail || "An error occurred while creating payment",
      )
    } finally {
  ...
  const currentPlan =
    subscriptionData?.subscription?.plan || plans.find((p) => p.code === "FREE")
            ...
            {plansLoading ? (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {[1, 2, 3].map((i) => (
                  <div
                    key={i}
                    className="h-[500px] rounded-2xl bg-muted/50 animate-pulse"
                  />
                ))}
              </div>
            ) : plans.length === 0 ? (
                      ...
                      >
                        ...
                        <div className="relative p-8 flex flex-col h-full">
                          ...
                          <Button
                            ...
                            onClick={() =>
                              !isCurrentPlan && handleUpgradeClick(plan)
                            }
                          ...
                          <div className="space-y-4 flex-1">
                            ...
                            {plan.monthly_credits !== null && (
                              <div className="flex items-center gap-3">
                                <div
                                  className={`p-1 rounded-full ${plan.is_featured ? "bg-primary/10" : "bg-muted"}`}
                                >
                                  <Check
                                    className={`w-4 h-4 ${plan.is_featured ? "text-primary" : "text-green-600 dark:text-green-400"}`}
                                  />
                                </div>
            ...
            <div className="w-full max-w-xl">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          ...
                          <span className="text-primary font-medium">
                            {currentPlan?.additional_credit_price
                              ? formatPrice(
                                  currentPlan.additional_credit_price,
              ...
              <div className="rounded-2xl bg-card border border-border p-8 space-y-8">
                ...
                <p className="text-xs text-muted-foreground text-center">
                  Credits will be added to your account immediately. No
                  expiration.
                </p>
              </div>
            </div>
            ...
            {historyLoading ? (
              ...
              <div className="space-y-4">
                <div className="text-sm text-muted-foreground mb-6">
                  Showing {paymentHistory.offset + 1} -{" "}
                  {Math.min(
                    paymentHistory.offset + paymentHistory.limit,
                    paymentHistory.total,
                  )}{" "}
                  of {paymentHistory.total} transactions
                </div>
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-6">
                          ...
                          <div
                            className={`flex items-center gap-2 px-3 py-1.5 rounded-full ${statusDisplay.bg}`}
                          >
                            <StatusIcon
