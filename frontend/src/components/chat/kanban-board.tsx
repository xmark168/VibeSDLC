  ...
  return (
    <div ref={setNodeRef} style={style} {...attributes} {...listeners}>
      <KanbanCard
        ...
        onMove={
          onCardMove
            ? (targetColumn) => onCardMove(card.id, targetColumn)
            : undefined
        }
...
function DroppableColumn({
  ...
  const _limitType = wipData?.limit_type ?? column.limitType

  const getWIPBadgeStyle = () => {
    if (!wipLimit)
      return "bg-slate-100 dark:bg-slate-900/50 text-slate-700 dark:text-slate-300 font-medium"
    ...
    <div className="w-72 flex-shrink-0">
      <div className="flex items-center justify-between mb-4 pb-3 border-b border-border/40">
        <div className="flex items-center gap-2.5">
          ...
          <span
            className={`text-xs px-2.5 py-1 rounded-lg ${getWIPBadgeStyle()}`}
          >
...
export function KanbanBoard({
  ...
  const [_clonedCards, setClonedCards] = useState<KanbanCardData[] | null>(null)
  ...
  const crossContainerTarget = useRef<{
    targetColumn: string
    targetIndex: number
    overId: string
  } | null>(null)
  ...
  useEffect(() => {
    if (dbKanbanData?.board) {
      ...
      const allCards: KanbanCardData[] = [
        ...filterItems(dbKanbanData.board.Todo || []).map((item: any) =>
          mapItem(item, "todo"),
        ),
        ...filterItems(dbKanbanData.board.InProgress || []).map((item: any) =>
          mapItem(item, "inprogress"),
        ),
        ...filterItems(dbKanbanData.board.Review || []).map((item: any) =>
          mapItem(item, "review"),
        ),
        ...filterItems(dbKanbanData.board.Done || []).map((item: any) =>
          mapItem(item, "done"),
        ),
        ...filterItems(dbKanbanData.board.Archived || []).map((item: any) =>
          mapItem(item, "archived"),
        ),
      ]
  ...
  useEffect(() => {
    const handleStoryStateChanged = (event: CustomEvent) => {
      ...
      setCards((prev) => {
        const updatedCards = prev.map((card) => {
          ...
          if (agent_state && agent_state !== "PENDING")
            updated.agent_sub_status = null
    ...
    window.addEventListener(
      "story-state-changed",
      handleStoryStateChanged as EventListener,
    )
    return () => {
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      ...
      handleStoryStatusChanged as EventListener,
    )
    return () => {
      window.removeEventListener(
        "story-status-changed",
  ...
  const checkDependenciesCompleted = useCallback(
    (
      ...
      const incompleteDeps = card.dependencies
        ...
        .filter(
          (dep): dep is KanbanCardData =>
            dep !== undefined &&
            dep.columnId !== "done" &&
            dep.columnId !== "archived",
        )
      ...
      return cardsToFilter.filter((card) => {
        ...
        if (selectedFilters.priorities.length > 0 && card.rank !== undefined) {
          matchesPriority = selectedFilters.priorities.some((priority) => {
            ...
            if (priority === "medium" && card.rank! > 3 && card.rank! <= 7)
              return true
  ...
  const handleDragOver = useCallback(
    (event: DragOverEvent) => {
      ...
      const originalColumn = clonedCardsRef.current?.find(
        (c) => c.id === active.id,
      )?.columnId
  ...
  const handleDragEnd = useCallback(
    async (event: DragEndEvent) => {
      const { active, over } = event
      const originalColumnId = clonedCardsRef.current?.find(
        (c) => c.id === active.id,
      )?.columnId
      ...
      if (crossContainerTarget.current) {
        ...
        const clonedCardsSnapshot = clonedCardsRef.current as
          | KanbanCardData[]
          | null
        if (clonedCardsSnapshot) {
          overContainer = clonedCardsSnapshot.find(
            (c: KanbanCardData) => c.id === overId,
          )?.columnId
        }
        if (!overContainer) {
      ...
      const statusMap: Record<
        string,
        "Todo" | "InProgress" | "Review" | "Done" | "Archived"
      > = {
        todo: "Todo",
        inprogress: "InProgress",
        review: "Review",
      ...
      if (activeContainer === overContainer) {
        ...
        const activeCard =
          savedClonedCards?.find((c) => c.id === active.id) ||
          cardsRef.current.find((c) => c.id === active.id)
        if (activeCard) {
        ...
        if (
          ...
          (overContainer === "review" || overContainer === "done")
        ) {
          toast.error(
            "Cannot move from Todo directly to Review/Done. Stories must go through InProgress first.",
          )
          if (savedClonedCards) {
        ...
        if (
          ...
          if (
            ...
            toast.error(
              `Cannot move to ${overContainer === "review" ? "Review" : "Done"}: Agent is still working on this story (${agentState})`,
            )
            if (savedClonedCards) {
              setCards(savedClonedCards)
            }
        ...
        if (
          activeCard &&
          overContainer !== "todo" &&
          overContainer !== "archived"
        ) {
          const { isBlocked, incompleteDeps } = checkDependenciesCompleted(
            activeCard,
            savedClonedCards || undefined,
          )
          if (isBlocked) {
            toast.error(
              `Cannot move story: ${incompleteDeps.length} incomplete dependencies`,
            )
          ...
          return updatedCards.map((card) => {
            ...
            const newIsBlocked =
              ...
              card.dependencies.some((depId) => {
                ...
                return (
                  dep && dep.columnId !== "done" && dep.columnId !== "archived"
                )
              })
            ...
            if (
              card.isBlocked === newIsBlocked &&
              card.blockedByCount === newBlockedByCount
            ) {
              return card
            }
            return {
    ...
    [projectId, checkDependenciesCompleted, cards.find],
  )
  ...
  const handleUpdateStory = useCallback(
    async (storyId: string, storyData: StoryFormData) => {
          ...
          prev.map((card) =>
            card.id === storyId
              ? {
                  ...
                  story_point:
                    updatedStory.story_point ?? storyData.story_point,
                  priority:
                    ...
                    (storyData.priority === "High"
                      ...
                      : storyData.priority === "Medium"
                        ...
                        : 3),
                  epic_id:
                    updatedStory.epic_id === null
                      ? undefined
                      : (updatedStory.epic_id ?? storyData.epic_id),
                  epic_code: updatedStory.epic_id
                    ? (updatedStory.epic_code ?? card.epic_code)
                    : undefined,
                  epic_title: updatedStory.epic_id
                    ? (updatedStory.epic_title ?? card.epic_title)
                    : undefined,
                  epic_description: updatedStory.epic_id
                    ? (updatedStory.epic_description ?? card.epic_description)
                    : undefined,
                  epic_domain: updatedStory.epic_id
                    ? (updatedStory.epic_domain ?? card.epic_domain)
                    : undefined,
                  acceptance_criteria:
                    updatedStory.acceptance_criteria ??
                    storyData.acceptance_criteria,
                  requirements:
                    updatedStory.requirements ?? storyData.requirements,
                  dependencies:
                    updatedStory.dependencies ?? storyData.dependencies,
  ...
  const handleMoveCard = useCallback(
    async (cardId: string, targetColumnId: string) => {
      ...
      if (targetColumnId !== "todo" && targetColumnId !== "archived") {
        ...
        if (isBlocked) {
          toast.error(
            `Không thể di chuyển story: ${incompleteDeps.length} dependencies chưa hoàn thành`,
          )
          return
        }
      }
      ...
      setCards((prev) => {
        ...
        return updatedCards.map((c) => {
          ...
          if (
            ...
            c.blockedByCount === newBlockedByCount
          ) {
            return c
          }
          return {
  ...
  const activeFilterCount =
    selectedFilters.types.length + selectedFilters.priorities.length
            ...
            <div className="p-4 rounded-lg bg-muted/30 border border-border/50 space-y-4">
              ...
              {activeFilterCount > 0 && (
                <Button
                  ...
                  size="sm"
                  onClick={() =>
                    setSelectedFilters({ types: [], priorities: [] })
                  }
