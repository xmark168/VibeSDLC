...
export const KanbanColumn = memo(
  ...
  (prevProps, nextProps) => {
    ...
    if (prevProps.column.cards.length !== nextProps.column.cards.length)
      return false
    ...
    for (let i = 0; i < prevProps.column.cards.length; i++) {
      ...
      if (
        JSON.stringify(prevCard.acceptance_criteria) !==
        JSON.stringify(nextCard.acceptance_criteria)
      )
        return false
      if (
        JSON.stringify(prevCard.requirements) !==
        JSON.stringify(nextCard.requirements)
      )
        return false
      if (
        JSON.stringify(prevCard.dependencies) !==
        JSON.stringify(nextCard.dependencies)
      )
        return false
    }
