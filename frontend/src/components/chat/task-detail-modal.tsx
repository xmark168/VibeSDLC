  ...
  if (!diffs || diffs.files.length === 0) {
    return (
      <div className="text-center text-muted-foreground py-8">
        ...
        {diffs?.base_branch && (
          <p className="text-xs mt-1">Comparing to {diffs.base_branch}</p>
        )}
      </div>
    )
  }
      ...
      <div className="flex items-center justify-between text-sm">
        <div className="flex items-center gap-3">
          ...
          <span className="text-green-600 font-medium">
            +{diffs.total_additions}
          </span>
          <span className="text-red-500 font-medium">
            -{diffs.total_deletions}
          </span>
        </div>
        <div className="flex items-center gap-2">
          {diffs.base_branch && (
            <span className="text-xs text-muted-foreground">
              vs {diffs.base_branch}
            </span>
          )}
          {onViewInFiles && (
            <Button
          ...
          {diffs.files.map((file, idx) => (
            ...
            >
              {file.status === "A" && (
                <Badge
                  variant="outline"
                  className="text-[10px] px-1.5 py-0 bg-green-500/10 text-green-600 border-green-500/30"
                >
                  A
                </Badge>
              )}
          ...
          {!runningPort ? (
            <div className="flex flex-col items-center justify-center h-full text-muted-foreground">
              ...
              <p className="text-sm mt-2">
                Start the dev server first using the "Dev Server" button
              </p>
            </div>
          ) : (
            <iframe
...
export function TaskDetailModal({
  ...
  const [selectedChild, setSelectedChild] = useState<KanbanCardData | null>(
    null,
  )
  ...
  const _getDependencyTitle = (depId: string): string => {
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("story-log", handleStoryLog as EventListener)
  }, [card?.id])
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-2">
                ...
                {card.type && (
                  <Badge
                    variant="outline"
                    className={getTypeBadgeColor(card.type)}
                  >
                ...
                <div className="mt-3 grid grid-cols-2 gap-x-6 gap-y-2 p-3 rounded-lg bg-muted/30 border text-xs">
                  {card.started_at && (
                    <>
                      ...
                      <span>
                        {new Date(card.started_at).toLocaleString("vi-VN")}
                      </span>
                    </>
                  )}
                  {agentState && (
                  ...
                  {card.merge_status !== undefined && (
                    <>
                      ...
                      <span className="flex items-center gap-1.5">
                        <span
                          className={`inline-block w-2 h-2 rounded-full ${
                            card.merge_status === "merged"
                              ? "bg-green-500"
                              : card.merge_status === "conflict"
                                ? "bg-destructive"
                                : "bg-amber-500"
                          }`}
                        />
        ...
        >
          <TabsList
            className={`grid w-full ${status === "Done" || status === "Archived" ? "grid-cols-2" : "grid-cols-3"}`}
          >
              ...
              {card.acceptance_criteria &&
                card.acceptance_criteria.length > 0 && (
                  <div>
                    ...
                    <ul className="space-y-1">
                      {card.acceptance_criteria.map(
                        (ac: string, idx: number) => (
                          <li
                            key={idx}
                            className="text-sm text-muted-foreground flex items-start gap-2"
                          >
                ...
                {card.status && (
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <Badge
                        variant="outline"
                        className={`w-fit ${getStatusBadgeColor(card.status)}`}
                      >
                  ...
                  <div className="space-y-2">
                    {card.children.map((child) => (
                      ...
                      >
                        <Badge
                          variant="outline"
                          className={getTypeBadgeColor(child.type)}
                        >
                        ...
                        {child.status && (
                          <Badge
                            variant="outline"
                            className={getStatusBadgeColor(child.status)}
                          >
                            {formatStatusName(child.status)}
                          </Badge>
                        )}
                        {child.story_point !== undefined &&
                          child.story_point !== null && (
              ...
              {storyLogs.length === 0 ? (
                ...
                storyLogs.map((log) => (
                  ...
                  >
                    ...
                    {log.node && (
                      <span
                        className="text-primary/70 shrink-0 w-28 truncate"
                        title={log.node}
                      >
                        [{log.node}]
                      </span>
            ...
            <div className="space-y-4">
              {card.worktree_path ? (
                ...
                <div className="text-center text-muted-foreground py-8">
                  ...
                  <p className="text-xs mt-1">
                    Diffs will appear once the agent starts working
                  </p>
                </div>
              )}
            </div>
