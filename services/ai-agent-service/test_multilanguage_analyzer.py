...
exec(
    open(
        "app/agents/developer/planner/tools/codebase_analyzer.py", encoding="utf-8"
    ).read()
)
...
def test_multilanguage_analyzer():
    ...
    try:
        ...
        for filename, expected in detector_tests:
            ...
            print(
                f"  {status} {filename} -> {detected.value} (expected: {expected.value})"
            )
        ...
        print(
            f"  ✅ Python analysis: {len(py_analysis.get('classes', []))} classes, {len(py_analysis.get('functions', []))} functions"
        )

        # Test JavaScript file
        js_analysis = analyzer.analyze_file_by_language("frontend/app.js")
        print(
            f"  ✅ JavaScript analysis: {len(js_analysis.get('classes', []))} classes, {len(js_analysis.get('functions', []))} functions"
        )

        # Test TypeScript file
        ts_analysis = analyzer.analyze_file_by_language("frontend/types.ts")
        print(
            f"  ✅ TypeScript analysis: {len(ts_analysis.get('classes', []))} classes, {len(ts_analysis.get('interfaces', []))} interfaces"
        )
