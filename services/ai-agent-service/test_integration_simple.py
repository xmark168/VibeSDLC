...
def test_integration():
    ...
    try:
        ...
        try:
            with open(
                "app/templates/prompts/developer/planner.py", "r", encoding="utf-8"
            ) as f:
            ...
            print(
                f"  {'✅' if has_placeholder else '❌'} Prompt template has codebase_context placeholder"
            )
            ...
            if has_placeholder:
                ...
                if start_idx != -1:
                    ...
                    if end_idx != -1:
                        ...
                        # Test formatting
                        mock_requirements = (
                            '{"task_id": "TEST-001", "description": "Test task"}'
                        )
                        try:
                            ...
                            print(
                                f"  ✅ Template formatting successful: {len(formatted)} chars"
                            )

                            # Check if context is properly included
                            context_included = "### File Structure:" in formatted
                            print(
                                f"  {'✅' if context_included else '❌'} Codebase context included in formatted prompt"
                            )
        ...
        benefits = [
            ...
            (
                "Configuration files",
                "pyproject.toml" in context or "Dockerfile" in context,
            ),
        ]
        ...
        # Final assessment
        total_checks = (
            len(key_elements) + 4 + len(benefits)
        )  # context + template + benefits
        passed_total = (
            passed_checks
            + (4 if has_placeholder else 0)
            + sum(1 for _, check in benefits if check)
        )
