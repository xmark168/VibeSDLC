...
def recover_password(email: str, session: SessionDep) -> Message:
    """
    Password Recovery - DISABLED (requires email configuration)
    """
    raise HTTPException(
        status_code=501,
        detail="Password recovery is not available. Email service is not configured."
    )
...
def reset_password(session: SessionDep, body: NewPassword) -> Message:
    """
    Reset password - DISABLED (requires email configuration)
    """
    raise HTTPException(
        status_code=501,
        detail="Password reset is not available. Email service is not configured."
    )
...
def recover_password_html_content(email: str, session: SessionDep) -> Any:
    """
    HTML Content for Password Recovery - DISABLED (requires email configuration)
    """
    raise HTTPException(
        status_code=501,
        detail="Password recovery is not available. Email service is not configured."
