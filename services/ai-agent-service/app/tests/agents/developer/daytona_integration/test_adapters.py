...
class TestLocalFilesystemAdapter:
    ...
    def test_read_file_with_line_range(
        self, temp_working_directory, sample_file_content
    ):
    ...
    def test_list_files(self, temp_working_directory):
        ...
        result = adapter.list_files(
            directory_path=".", working_directory=str(temp_working_directory)
        )
...
class TestDaytonaFilesystemAdapter:
    ...
    def test_error_handling(self, mock_sandbox_manager, mock_sandbox):
        ...
        # Setup
        adapter = DaytonaFilesystemAdapter(mock_sandbox_manager)
        mock_sandbox.fs.download_file.side_effect = Exception(
        ...
        assert "Sandbox connection failed" in str(
            exc_info.value
        ) or "Failed to read file" in str(exc_info.value)
...
class TestLocalGitAdapter:
    ...
    def test_commit(self, temp_git_repo):
        ...
        result = adapter.commit(
            message="feat: Add test file", working_directory=str(temp_git_repo)
        )
...
class TestFactoryFunctions:
    ...
    def test_get_filesystem_adapter_daytona_mode(
        self, mock_get_manager, mock_env_daytona_enabled, mock_sandbox_manager
    ):
    ...
    def test_get_git_adapter_daytona_mode(
        self, mock_get_manager, mock_env_daytona_enabled, mock_sandbox_manager
    ):
