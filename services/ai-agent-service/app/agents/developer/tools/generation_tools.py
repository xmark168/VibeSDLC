...
def select_integration_strategy_tool(
    task_description: str, codebase_context: str, target_files: List[str] = None
) -> str:
    """
    ...
    try:
        ...
        # Analyze task type
        if any(
            keyword in task_lower for keyword in ["add", "implement", "create", "build"]
        ):
            if any(
                keyword in task_lower
                for keyword in ["endpoint", "route", "api", "method", "function"]
            ):
                strategies["extend_existing"]["score"] += 3
                strategies["extend_existing"]["reasons"].append(
                    "Adding new endpoints/methods to existing structure"
                )
            elif any(
                keyword in task_lower
                for keyword in ["service", "module", "component", "class"]
            ):
                strategies["create_new"]["score"] += 3
                strategies["create_new"]["reasons"].append(
                    "Creating new service/module/component"
                )
            else:
        ...
        if any(
            keyword in task_lower
            for keyword in ["fix", "bug", "error", "issue", "problem"]
        ):
            strategies["fix_issue"]["score"] += 4
            strategies["fix_issue"]["reasons"].append(
                "Task involves fixing bugs or issues"
            )
        ...
        if any(
            ...
            strategies["refactor"]["reasons"].append(
                "Task involves code improvement or restructuring"
            )

        if any(
            keyword in task_lower
            for keyword in ["update", "modify", "change", "enhance"]
        ):
        ...
        if "existing" in context_lower and any(
            keyword in context_lower
            for keyword in ["models", "routes", "controllers", "services"]
        ):
            strategies["extend_existing"]["score"] += 2
            strategies["extend_existing"]["reasons"].append(
                "Existing structure detected that can be extended"
            )

        if any(keyword in context_lower for keyword in ["legacy", "old", "outdated"]):
            strategies["refactor"]["score"] += 2
            strategies["refactor"]["reasons"].append(
                "Legacy code detected that may benefit from refactoring"
            )

        if "empty" in context_lower or "new project" in context_lower:
            strategies["create_new"]["score"] += 3
            strategies["create_new"]["reasons"].append(
                "New or empty project - creating new structure"
            )
        ...
        if target_files:
            ...
            if existing_files:
                ...
                strategies["extend_existing"]["reasons"].append(
                    f"Found {len(existing_files)} existing files to extend"
                )

            if new_files:
                strategies["create_new"]["score"] += len(new_files)
                strategies["create_new"]["reasons"].append(
                    f"Need to create {len(new_files)} new files"
                )
        ...
        sorted_strategies = sorted(
            strategies.items(), key=lambda x: x[1]["score"], reverse=True
        )
        if (
            len(sorted_strategies) >= 2
            and sorted_strategies[0][1]["score"] - sorted_strategies[1][1]["score"] <= 1
        ):
        ...
        guidance = generate_implementation_guidance(
            strategy_name, task_description, codebase_context
        )
...
def analyze_task_type(task_description: str) -> str:
    ...
    if any(
        keyword in task_lower
        for keyword in ["auth", "login", "register", "jwt", "token"]
    ):
        ...
        keyword in task_lower
        for keyword in ["database", "model", "schema", "migration"]
    ):
        return "database"
    elif any(
        keyword in task_lower for keyword in ["ui", "frontend", "component", "page"]
    ):
        return "frontend"
    elif any(
        keyword in task_lower
        ...
        keyword in task_lower for keyword in ["deploy", "deployment", "docker", "ci/cd"]
    ):
        return "deployment"
...
def generate_implementation_guidance(
    strategy: str, task_description: str, codebase_context: str
) -> Dict[str, Any]:
    ...
    if strategy == "extend_existing":
        guidance["approach"] = (
            "Extend existing codebase by adding new functionality to current files and structures"
        )
        guidance["steps"] = [
        ...
        guidance["testing_strategy"] = (
            "Add unit tests for new functionality, run existing tests to ensure no regression"
        )

    elif strategy == "create_new":
        guidance["approach"] = (
            "Create new files, modules, or components to implement the functionality"
        )
        guidance["steps"] = [
        ...
        guidance["testing_strategy"] = (
            "Create comprehensive test suite for new components, including unit and integration tests"
        )
        ...
        guidance["testing_strategy"] = (
            "Ensure 100% test coverage before refactoring, run tests after each change"
        )

    elif strategy == "fix_issue":
        guidance["approach"] = (
            "Identify and fix specific bugs or issues with minimal code changes"
        )
        guidance["steps"] = [
        ...
        guidance["testing_strategy"] = (
            "Create specific test cases that reproduce the bug, verify fix with comprehensive testing"
        )
        ...
        guidance["testing_strategy"] = (
            "Test each component individually, then perform comprehensive integration testing"
        )
