...
def finalize(state: ImplementorState) -> ImplementorState:
    ...
    try:
        ...
        print(f"  📊 Summary: {len(state.files_created)} created, {len(state.files_modified)} modified")
        print(f"  🌿 Branch: {state.feature_branch}")
        print(f"  💾 Commit: {state.final_commit_hash[:8] if state.final_commit_hash else 'N/A'}")
        ...
        message = AIMessage(
            content=f"❌ Finalization error: {str(e)}"
        )
        state.messages.append(message)
...
def _generate_final_summary(state: ImplementorState) -> dict:
    ...
    summary = {
        ...
        "implementation_type": "new_project" if state.is_new_project else "existing_project",
        ...
        # Testing
        "tests_run": hasattr(state.test_execution, 'test_command') and bool(state.test_execution.test_command),
        ...
        # Next steps for user
        "next_steps": [
            "Visit Git platform to create Pull Request" if state.status == "pr_ready" else None,
    ...
    # Remove None values
    summary["phases_completed"] = [p for p in summary["phases_completed"] if p is not None]
