...
def create_feature_branch_tool(
    ...
    try:
        ...
        if not working_dir.exists():
            return json.dumps(
                {"status": "error", "message": f"Directory '{working_directory}' does not exist"}
            )
        ...
        if safe_branch_name in [head.name for head in repo.heads]:
            return json.dumps(
                {"status": "error", "message": f"Branch '{safe_branch_name}' already exists"}
            )
        ...
        if initial_commit_created:
            ...
            try:
                ...
                return json.dumps(
                    {"status": "error", "message": f"Base branch '{base_branch}' does not exist"}
                )
        ...
        return json.dumps({"status": "error", "message": f"Error creating branch: {str(e)}"})
...
def commit_changes_tool(
    ...
    try:
        ...
        for file_path, stats in commit.stats.files.items():
            stats_lines.append(f"{file_path} | {stats['insertions']}+ {stats['deletions']}-")
        commit_stats = "\n".join(stats_lines)
        ...
        return json.dumps({"status": "error", "message": f"Error committing changes: {str(e)}"})
...
def create_pull_request_tool(
    ...
    try:
        ...
        try:
            ...
            return json.dumps(
                {"status": "error", "message": f"'{working_directory}' is not a Git repository"}
            )
        ...
        if current_branch == base_branch:
            return json.dumps(
                {"status": "error", "message": f"Cannot create PR from base branch '{base_branch}'"}
            )
        ...
        try:
            ...
            return json.dumps({"status": "error", "message": "No 'origin' remote found"})
        ...
        try:
            ...
            if push_info and push_info[0].flags & push_info[0].ERROR:
                push_status = "error"
                return json.dumps(
                    {"status": "error", "message": f"Push failed: {push_info[0].summary}"}
                )
        except GitCommandError:
            # Try to set upstream and push
            try:
                push_info = origin.push(f"{current_branch}:{current_branch}", set_upstream=True)
                push_status = "success_with_upstream"
            except GitCommandError as e2:
                return json.dumps({"status": "error", "message": f"Push failed: {str(e2)}"})
        ...
        return json.dumps({"status": "error", "message": f"Error creating pull request: {str(e)}"})
