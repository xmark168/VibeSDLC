...
@tool
def ast_parser_tool(file_path: str, analysis_type: str = "structure") -> str:
    """
    ...
    try:
        ...
        result = {"file_path": file_path, "analysis_type": analysis_type}
        ...
        return json.dumps(
            {"error": f"AST parsing failed: {str(e)}", "file_path": file_path}, indent=2
        )


@tool
def file_analyzer_tool(file_path: str, analysis_depth: str = "basic") -> str:
    """
    ...
    try:
        ...
        return json.dumps(
            {"error": f"File analysis failed: {str(e)}", "file_path": file_path},
            indent=2,
        )
