...
def dependency_analyzer_tool(
    target_path: str, analysis_scope: str = "all", depth: int = 2
) -> str:
    """
...
def execution_order_tool(
    tasks: List[Dict[str, Any]], dependency_rules: List[Dict[str, Any]] = None
) -> str:
    """
    ...
    try:
        if not tasks:
            return json.dumps(
                {"execution_order": [], "error": "No tasks provided"}, indent=2
            )
        ...
        result = {
            ...
            "dependency_violations": check_dependency_violations(
                    ...
                    if isinstance(node, ast.Import):
                        ...
                        if is_internal_module(module):
                            ...
                            external_deps.append(
                                {
                                    "package": (
                                        module.split(".")[0] if module else "unknown"
                                    ),
                                    "module": module,
                                    "type": "from_import",
                                    "imports": [alias.name for alias in node.names],
...
def find_parallel_groups(
    task_graph: Dict[str, Dict[str, Any]], execution_order: List[Dict[str, Any]]
) -> List[List[str]]:
...
def calculate_execution_phases(
    task_graph: Dict[str, Dict[str, Any]], execution_order: List[Dict[str, Any]]
) -> List[Dict[str, Any]]:
    ...
    for item in execution_order:
        ...
        if all(dep in completed_tasks for dep in dependencies):
            ...
            # Start new phase
            if current_phase:
                phases.append({"phase": len(phases) + 1, "tasks": current_phase.copy()})
    ...
    # Add final phase
    if current_phase:
        phases.append({"phase": len(phases) + 1, "tasks": current_phase})

    return phases


def check_dependency_violations(
    task_graph: Dict[str, Dict[str, Any]], execution_order: List[Dict[str, Any]]
) -> List[str]:
    ...
    for item in execution_order:
        ...
        for dep in dependencies:
            if dep not in executed_tasks:
                violations.append(
                    f"Task {task_id} depends on {dep} but {dep} is not executed yet"
                )
