...
@tool
def task_parser_tool(task_description: str, parsing_mode: str = "comprehensive") -> str:
    """
    ...
    try:
        ...
        if parsing_mode == "comprehensive":
            ...
            result["functional_requirements"] = extract_functional_requirements(
                task_description
            )
            result["acceptance_criteria"] = extract_acceptance_criteria(
                task_description
            )
...
def effort_estimation_tool(
    task_info: Dict[str, Any], estimation_method: str = "story_points"
) -> str:
    """
...
@tool
def risk_assessment_tool(task_info: Dict[str, Any], codebase_context: str = "") -> str:
    """
...
def determine_task_type(description: str) -> str:
    ...
    if any(word in description_lower for word in ["bug", "fix", "error", "issue"]):
        ...
        word in description_lower for word in ["ui", "frontend", "interface", "design"]
    ):
        return "ui_change"
    elif any(
        word in description_lower for word in ["integrate", "integration", "connect"]
    ):
        return "integration"
    elif any(
        word in description_lower for word in ["infrastructure", "deploy", "config"]
    ):
...
def extract_priority(description: str) -> str:
    ...
    if any(
        word in description_lower for word in ["urgent", "critical", "high priority"]
    ):
        return "high"
...
def extract_user_roles(description: str) -> List[str]:
    """Extract user roles."""
    role_patterns = [r"as\s+a\s+([^,]+)", r"user\s+([^,]+)", r"role:\s*([^.]+)"]
...
def assess_technical_risks(
    task_info: Dict[str, Any], codebase_context: str
) -> List[Dict[str, Any]]:
