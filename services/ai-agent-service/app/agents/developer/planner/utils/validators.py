...
def validate_dependency_mapping(
    ...
    if len(execution_order) > 3 and not parallel_opportunities:
        issues.append(
            "Consider identifying parallel execution opportunities for efficiency"
        )
...
def validate_implementation_plan(
    ...
    if (
        not isinstance(complexity_score, int)
...
def calculate_validation_score(
    ...
    weights = {"completeness": 0.35, "consistency": 0.35, "effort": 0.20, "risk": 0.10}
...
def validate_plan_readiness(
    ...
    completeness_score = 1.0 - (
        len([i for i in plan_issues if "missing" in i.lower()]) * 0.2
    )
    consistency_score = 1.0 - (
        len([i for i in plan_issues if "mismatch" in i.lower()]) * 0.3
    )
    effort_score = 1.0 - (
        len([i for i in plan_issues if "hours" in i.lower() or "effort" in i.lower()])
        * 0.25
    )
