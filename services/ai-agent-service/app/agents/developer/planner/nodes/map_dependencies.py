...
def map_dependencies(state: PlannerState) -> PlannerState:
    ...
    try:
        ...
        # Model changes next
        model_files = [f for f in codebase_analysis.files_to_modify if "models" in f["path"]]
        if model_files:
        ...
        service_files = [f for f in codebase_analysis.files_to_create if "services" in f["path"]]
        if service_files:
            depends_on = []
            if model_files:
        ...
        # API endpoints
        api_files = [f for f in codebase_analysis.files_to_modify if "api" in f["path"] or "routes" in f["path"]]
        ...
        if codebase_analysis.testing_requirements:
            ...
            execution_order.append({
                ...
                "blocking": False,
                "depends_on": depends_on,
                "files": codebase_analysis.testing_requirements.get("unit_tests", {}).get("files", [])
            })
        ...
        non_blocking_steps = [step["step"] for step in execution_order if not step["blocking"]]
