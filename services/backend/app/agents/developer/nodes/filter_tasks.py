...
def _find_backlog_item_by_id(
    backlog_data: list[dict[str, Any]], item_id: str
) -> dict[str, Any] | None:
    """
...
def _resolve_parent_context(
    item: dict[str, Any], backlog_data: list[dict[str, Any]]
) -> str:
    """
...
def _detect_scope(item: dict[str, Any]) -> str:
    """
    Detect task scope (backend, frontend, or full-stack) from labels.
    ...
    has_backend = "backend" in labels_lower
    has_frontend = "frontend" in labels_lower
    ...
    if has_backend and has_frontend:
        ...
        # Default to full-stack if no scope labels found
        return "full-stack"
...
def filter_tasks(state: DeveloperState) -> DeveloperState:
    ...
    for item_id in assigned_items:
        ...
        if task_type not in ["Development", "Infrastructure"]:
            print(
                f"⏭️ Skipping {item_id} ({item.get('type', 'Unknown')}): task_type='{task_type}'"
            )
            continue
        ...
        # Detect scope from labels
        scope = _detect_scope(item)
        ...
        print(
            f"✅ Eligible: {item_id} ({task_type}) [Scope: {scope}] - {item.get('title', '')[:50]}..."
        )
        if parent_context:
            parent_title = (
                parent_context.split("\n")[0] if parent_context else "No parent"
            )
