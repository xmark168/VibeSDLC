            ...
            for file in files:
                ...
                if file in stack_patterns:
                    ...
                    if "language" in pattern:
                        ...
                        detected_languages[lang] = max(
                            detected_languages.get(lang, 0), confidence
                        )
    ...
    if primary_lang == "Python":
        if not frameworks:
            recommendations.append(
                "Consider using FastAPI for modern web APIs or Flask for simpler applications"
            )
        if "FastAPI" in frameworks:
            recommendations.append(
                "FastAPI detected - consider adding Pydantic models and automatic API documentation"
            )
        recommendations.append(
            "Consider using virtual environments (venv or conda) for dependency management"
        )

    elif primary_lang == "JavaScript":
        if "React" in frameworks:
            recommendations.append(
                "React detected - consider using TypeScript for better type safety"
            )
        if not any(fw in frameworks for fw in ["React", "Vue.js", "Angular"]):
            recommendations.append(
                "Consider using a modern frontend framework like React, Vue.js, or Angular"
            )

    if not stack_info.get("databases"):
        recommendations.append(
            "No database detected - consider adding database integration if needed"
        )
...
def retrieve_boilerplate_tool(
    ...
    try:
        ...
        if boilerplate_path is None:
            ...
            boilerplate_path = (
                current_file.parent.parent.parent.parent / "templates" / "boilerplate"
            )
        else:
