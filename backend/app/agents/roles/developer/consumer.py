"""Developer Kafka consumer.

Consumes agent routing events and triggers Developer crew execution.
"""
...
class DeveloperConsumer:
    """Kafka consumer for Developer crew.

    Consumes:
    - AgentRoutingEvent from agent.routing topic (where to_agent = "developer")

    Triggers:
    - DeveloperCrew.execute() for each delegation
    - Publishes AgentResponseEvent with implementation results
    """

    def __init__(self, group_id: str = "developer-consumer"):
    ...
    async def handle_routing_event(self, event_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if to_agent != "developer":
                logger.debug(f"Routing event not for Developer, skipping (to_agent={to_agent})")
                return

            logger.info(f"Developer received delegation from {event_data.get('from_agent')}")
            ...
            if result.get("success"):
                logger.info(f"Developer completed task successfully")
            else:
    ...
    async def start(self) -> None:
        ...
        if self._running:
            logger.warning("Developer consumer already running")
            return
