...
Consumes agent routing events and triggers Tester crew execution.
"""
...
class TesterConsumer:
    """Kafka consumer for Tester crew.

    Consumes:
    - AgentRoutingEvent from agent.routing topic (where to_agent = "tester")

    Triggers:
    - TesterCrew.execute() for each delegation
    - Publishes AgentResponseEvent with test plan/cases
    """

    def __init__(self, group_id: str = "tester-consumer"):
    ...
    async def handle_routing_event(self, event_data: Any) -> None:
        ...
        try:
            ...
            if to_agent != "tester":
                logger.debug(f"Routing event not for Tester, skipping (to_agent={to_agent})")
                return

            logger.info(f"Tester received delegation from {event_data.get('from_agent')}")
            ...
            if result.get("success"):
                logger.info(f"Tester completed task successfully")
            else:
    ...
    async def start(self) -> None:
        ...
        if self._running:
            logger.warning("Tester consumer already running")
            return
