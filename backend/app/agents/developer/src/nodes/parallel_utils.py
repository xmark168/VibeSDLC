...
def group_steps_by_layer(steps: List[Dict]) -> Dict[float, List[Dict]]:
    ...
    for step in steps:
        ...
        # Adjust layer for components AND pages with component dependencies
        # This ensures components run AFTER their dependencies (no parallel conflicts)
        if has_component_dep and base_layer >= 6:
