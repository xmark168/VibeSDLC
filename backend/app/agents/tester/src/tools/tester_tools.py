...
def _detect_test_command(project_path: Path, test_type: str, test_file: str = "") -> str:
        ...
        try:
            ...
            for variant in type_variants:
                if variant in scripts:
                    return f"{run_cmd} {variant}"
            
            # Fallback to generic test script
            if "test" in scripts:
                return f"{run_cmd} test"
    ...
    if (project_path / "pyproject.toml").exists() or \
       (project_path / "pytest.ini").exists():
        ...
        for dir_name in ["tests/integration", "tests/unit", "tests"]:
            if test_type in dir_name or test_type == "all":
                if (project_path / dir_name).exists():
                    return f"pytest {dir_name} -v"
