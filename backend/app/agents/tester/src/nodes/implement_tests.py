...
async def _execute_llm_with_tools(
    ...
    config = {"callbacks": [handler], "run_name": name} if handler else {"run_name": name}
        ...
        for tool_call in response.tool_calls:
            ...
            if tool_name in explore_tools:
                ...
                if explore_count > max_explore:
                    logger.warning(f"[_execute_llm_with_tools] Too many explore calls ({explore_count})")
    ...
    logger.warning(f"[_execute_llm_with_tools] Max iterations reached")
